      ...
      try {
        ...
        if (text.startsWith('/remind ')) {
          ...
          if (spaceIdx === -1) {
            await bot.sendMessage(
              chatId,
              `❌ Format: <code>/remind &lt;durée&gt; &lt;message&gt;</code>\nExemple: <code>/remind 10min Appeler maman</code>`,
              { parse_mode: 'HTML' }
            );
            return;
          }
          ...
          if (durationMs === -1 || durationMs < 10000) {
            await bot.sendMessage(
              chatId,
              `❌ Durée invalide ou trop courte (min 10 secondes).\n\nFormats supportés: 30s, 10min, 2h, 1j`,
              { parse_mode: 'HTML' }
            );
            return;
          }
          ...
          // Vérifier limite de rappels actifs par utilisateur (max 10)
          ...
          await botDoc.addLog('info', `Rappel créé par ${firstName}: "${reminderMsg}" dans ${humanDuration}`);
        ...
        if (text === '/reminders') {
          ...
          reminders.forEach((r, idx) => {
            const date = r.scheduledAt.toLocaleString('fr-FR');
            response += `${idx + 1}. ⏰ ${date}\n   📝 ${r.message}\n\n`;
          });
        ...
        console.error('ReminderBot error:', error.message);
