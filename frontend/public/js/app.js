...
const TEMPLATES = {
  ...
  ai: {
    ...
    configFields: [
      ...
      { key: 'ai_model',       label: 'Modèle (laisser vide = auto)', type: 'text', placeholder: 'gpt-4o-mini / mistral-small / llama3-8b-8192' },
      { key: 'ai_endpoint',    label: 'Endpoint custom (optionnel)', type: 'text', placeholder: 'https://api.openai.com/v1/chat/completions' },
  ...
  api: {
    ...
    desc: 'Connecte ton bot à n\'importe quelle API externe',
    badge: 'PRO',
    configFields: [
      { key: 'api_triggers',          label: 'Mots-clés déclencheurs', type: 'text', placeholder: 'météo, weather, temp', hint: 'Séparés par des virgules' },
      { key: 'api_welcome_message',   label: 'Message de bienvenue (/start)', type: 'textarea', placeholder: '🔗 Envoyez un mot-clé pour déclencher l\'appel API.' },
      { key: 'api_method',            label: 'Méthode HTTP', type: 'select', options: [
        { value: 'GET',    label: 'GET' },
        { value: 'POST',   label: 'POST' },
        { value: 'PUT',    label: 'PUT' },
        { value: 'DELETE', label: 'DELETE' },
      ]},
      { key: 'api_url',               label: 'URL de l\'API', type: 'text', placeholder: 'https://api.open-meteo.com/v1/forecast?q={query}', hint: 'Variables: {query}, {raw_query}, {user_id}, {first_name}' },
      { key: 'api_headers',           label: 'Headers HTTP', type: 'textarea', placeholder: 'Authorization: Bearer TOKEN\nX-API-Key: mykey', hint: 'Un header par ligne: Nom: Valeur' },
      { key: 'api_body',              label: 'Body de la requête (POST/PUT)', type: 'textarea', placeholder: '{"query": "{raw_query}"}', hint: 'JSON. Variables: {query}, {raw_query}' },
      { key: 'api_response_path',     label: 'JSON Path de la réponse', type: 'text', placeholder: 'current.temperature_2m', hint: 'Chemin pour extraire une valeur. Vide = réponse complète' },
      { key: 'api_response_template', label: 'Template de réponse', type: 'textarea', placeholder: '🌡️ Température : {result}°C', hint: 'Variables: {result}, {query}, {first_name}' },
      { key: 'api_fallback_message',  label: 'Message d\'erreur', type: 'text', placeholder: '❌ Impossible de récupérer les données.' },
      { key: 'api_buttons',           label: 'Boutons de réponse (JSON)', type: 'textarea', placeholder: '[[{"text":"Voir plus","type":"url","value":"https://..."}]]', hint: 'Tableau 2D de boutons inline. Laisser vide = pas de boutons.' },
      { key: 'api_cooldown_ms',       label: 'Cooldown anti-spam (ms)', type: 'text', placeholder: '2000' },
    ],
  },
};
